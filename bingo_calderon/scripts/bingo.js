    var areThereCards = 0; // checks if there are any generated cards
    ...
    class BingoMachine {
        ...
        draw() {
            if(areThereCards == 0){
                return alert("You Have NOT Yet Placed Any Cards");
            }
    ...
    function checkLuckyCards() {
        // check if any of the cards matches the lucky cards templates
        let hasWinner = false;
    ...
    function render() {
                ...
                return `<div class="col-md-4">
                    <table class="table table-bordered">
                        <thead class="table-danger">
                            <th class = "bg-warning">B</th>
                            <th class = "bg-warning">I</th>
                            <th class = "bg-warning">N</th>
                            <th class = "bg-warning">G</th>
                            <th class = "bg-warning">O</th>
                        </thead>
                        <tbody>
                        ${rows.map((row) => {
                            return `<tr>
        ...
        luckyCardsPlaceholderElem.innerHTML = luckyCards.map((luckyCard) => {
            return `<table class="table table-bordered">
                <thead class="table-danger">
                    ...
                    <th class = "bg-danger">I</th>
                    <th class = "bg-danger">N</th>
                    <th class = "bg-danger">G</th>
                    <th class = "bg-danger">O</th>
                </thead>
                <tbody>
                    ${luckyCard.map((row) => {
                        return `<tr>
                            <td class="${row[0] ? 'bg-warning' : ''}">&nbsp;</td>
                            <td class="${row[1] ? 'bg-warning' : ''}">&nbsp;</td>
                            <td class="${row[2] ? 'bg-warning' : ''}">&nbsp;</td>
                            <td class="${row[3] ? 'bg-warning' : ''}">&nbsp;</td>
                            <td class="${row[4] ? 'bg-warning' : ''}">&nbsp;</td>
                        </tr>`;
                    }).join('')}
                </tbody>
            </table>`;
        ...
        if(areThereCards > 1){
        document.getElementById('drawnBallsPlaceholder').innerHTML = nabola.map((bola) => `<span class="badge bg-danger  mb-1">${bola.letter}<br>${bola.number}</span>`).join(' ');
        }
    }
    ...
    drawBtn.addEventListener('click', () => {
        //draw a ball from tambiolo
        const bola = tambiolo.draw();

        //add drawn ball to nabola
        nabola.push(bola);

        // check all cards with cells is marked
        cards.forEach((card) => card.markBall(bola));

        // check lucky cards BINGO (if any)
