...
function renderBingoCard(cardData, cardIndex) {
    ...
    html += `
        ...
        <div class="d-flex justify-content-between mt-2">
            ...
            <button class="btn btn-outline-primary btn-sm rounded-pill shadow-sm" onclick="autoMarkCard(${cardIndex})">
                <i class="fa fa-magic"></i> Auto Mark Called
            </button>
        </div>
    </div>
    `;
...
function toggleCellMark(cardIdx, row, col) {
    ...
    updateCellUI(cardIdx, row, col);
    checkWin(cardIdx);
}

function updateCellUI(cardIdx, row, col) {
    const card = window.bingoCards[cardIdx];
    const cellId = `card${cardIdx}_cell${row}_${col}`;
    const cell = document.getElementById(cellId);
    if (card.marks[row][col]) {
        cell.classList.add('marked');
        cell.style.background = 'linear-gradient(90deg,#dbeafe,#e0e7ff)';
...
function checkWin(cardIdx) {
    ...
    // Rows & Columns
    for (let i = 0; i < 5; i++) {
        if (allTrue(marks[i])) return showWin(cardIdx, 'Row');
        if (allTrue(marks.map(row => row[i]))) return showWin(cardIdx, 'Column');
    }
    // Diagonals
    if (allTrue([0,1,2,3,4].map(i => marks[i][i]))) return showWin(cardIdx, 'Diagonal');
    if (allTrue([0,1,2,3,4].map(i => marks[i][4 - i]))) return showWin(cardIdx, 'Diagonal');
    // Blackout
    if (allTrue(marks.flat())) return showWin(cardIdx, 'Blackout');
}
...
// ------------- NEW: Called numbers UI and logic -------------

// Reset the called numbers and available numbers pool
function resetCalledNumbers() {
    calledNumbers = new Set();
    availableNumbers = [];
    for (let n = 1; n <= 75; n++) availableNumbers.push(n);
}

// Render the called numbers list
function renderCalledNumbers() {
    let container = document.getElementById('calledNumbersContainer');
    if (!container) {
        container = document.createElement('div');
        container.id = 'calledNumbersContainer';
        container.style.maxWidth = '500px';
        container.style.margin = '1rem auto';
        container.style.padding = '1rem';
        container.style.background = '#f1f5f9';
        container.style.borderRadius = '1rem';
        container.style.boxShadow = '0 4px 12px rgba(60,60,60,0.1)';
        container.innerHTML = '<h5>Called Numbers</h5><div id="calledNumbersList" style="display:flex;flex-wrap:wrap;gap:0.5rem;"></div>';
        document.querySelector('.container').appendChild(container);

        // Add call next number button
        const callButton = document.createElement('button');
        callButton.textContent = 'Call Next Number';
        callButton.className = 'btn btn-primary mb-2';
        callButton.onclick = callNextNumber;
        container.insertBefore(callButton, container.children[1]);
    }
    updateCalledNumbersList();
}

// Update the called numbers list UI
function updateCalledNumbersList() {
    ...
    for (const num of sortedNumbers) {
        const colLetter = getColumnForNumber(num);
        const span = document.createElement('span');
        span.textContent = `${colLetter}${num}`;
...
// Given a number, return its Bingo column letter
function getColumnForNumber(num) {
    for (const col in BINGO_RANGES) {
...
// Call next number randomly from availableNumbers
...
// Auto mark all cards with the called number
function autoMarkAllCards(number) {
    window.bingoCards.forEach((card, cardIdx) => {
        for (let col = 0; col < 5; col++) {
            for (let row = 0; row < 5; row++) {
                if (card.data[col][row] === number) {
...
// Optional helper: Manual auto-mark all called numbers on a specific card
function autoMarkCard(cardIdx) {
    const card = window.bingoCards[cardIdx];
    for (let col = 0; col < 5; col++) {
        for (let row = 0; row < 5; row++) {
            if (calledNumbers.has(card.data[col][row])) {
                card.marks[row][col] = true;
                updateCellUI(cardIdx, row, col);
            }
        }
