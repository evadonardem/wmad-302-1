...
import { Star, Settings, RefreshCw, Trophy, Award, Target, BarChart3, Play, Filter, X } from 'lucide-react';
...
const Dashboard = ({ difficulty = 'any', questionType = 'any' }: DashboardProps) => {
    ...
    useEffect(() => {
        ...
        if (savedCategoryId) {
            try {
                const parsedId = JSON.parse(savedCategoryId);
                const parsedName = savedCategoryName ? JSON.parse(savedCategoryName) : null;
                setSelectedCategory(parsedId);
                setSelectedCategoryName(parsedName);
            } catch (err) {
                console.error("Error parsing saved category from Categories page:", err);
            }
        }
    }, []);
    ...
    const fetchQuestions = async () => {
        try {
            ...
            // Use selectedCategory from Categories page if available
                    ...
                    <Card className="shadow-2xl border-0 bg-white">
                        <CardContent className="p-8 text-center">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                <Card className="border border-green-100 bg-white shadow-lg">
                                    <CardContent className="p-4">
                                        <div className="text-3xl font-bold text-green-600">{score}</div>
                                        <div className="text-sm text-green-700 font-medium">Correct Answers</div>
                                    </CardContent>
                                </Card>
                                <Card className="border border-red-100 bg-white shadow-lg">
                                    <CardContent className="p-4">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                <Button onClick={resetQuiz} variant="default" className="px-8 py-3">
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4">
            <div className="max-w-7xl mx-auto">
                {/* Header */}
                <Card className="shadow-xl border-0 bg-white text-slate-900 mb-8">
                    <CardContent className="p-8">
                        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                            <div className="flex-1">
                                <h1 className="text-4xl md:text-5xl font-bold mb-2">Quiz Challenge</h1>
                                <p style={{ color: "black" }} className="text-blue-100 text-lg md:text-xl">
                                    {selectedCategoryName 
                                        ? `Test your knowledge in ${selectedCategoryName}`
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 p-3 bg-green-50 rounded-lg border border-green-200">
                                        <span className="font-medium text-green-800">
                                            {difficulty && difficulty !== 'any' ? difficulty.charAt(0).toUpperCase() + difficulty.slice(1) : 'All Levels'}
                                        </span>
                                    </div>
                                </div>
                                <div>
